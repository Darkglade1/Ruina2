...
public class Courage() : Ruina2Power
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars => [new("AttackedCount",0), new ("AttackedThreshold", 1)];
        ...
        CardModel? cardSource)
    {
        if (dealer != null && props.IsPoweredAttack() && target.Monster != null && target.Monster is RoadHome && target.IsAlive)
        {
            ...
            if (DynamicVars["AttackedCount"].BaseValue >= DynamicVars["AttackedThreshold"].BaseValue)
            {
                Flash();
                DynamicVars["AttackedCount"].BaseValue = 0;
                await PowerCmd.Apply<CourageTempStr>(new ThrowingPlayerChoiceContext(), Owner, Amount, Owner,  null);
            }
        }
    }
}