...
public sealed class ButterfliesNormal : CustomEncounterModel
{
    public ButterfliesNormal() : base(RoomType.Monster)
    {
    }
    ...
    protected override IReadOnlyList<(MonsterModel, string?)> GenerateMonsters()
    {
        return new List<(MonsterModel, string?)>
        {
            ...
            (ModelDb.Monster<Butterflies>().ToMutable(), "slot4"),
            (ModelDb.Monster<Butterflies>().ToMutable(), "slot5")
        };
    }
}