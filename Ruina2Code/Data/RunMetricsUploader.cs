...
namespace Ruina2.Ruina2Code.Data;
...
public sealed class RunMetricsUploader<TPayload>
{
    ...
    private bool ShouldUpload(SerializableRun run, ulong localPlayerId)
    {
        ...
        if (!ShouldUploadMetrics()) return false;
        ...
        if (_config.IsOwnCharacter is { } isOwn)
        {
            ...
            {
                log?.Info($"[{mod}] Skipping metrics upload, active character isn't allowed.");
                return false;
            }
        }
    ...
    public bool ShouldUploadMetrics()
    {
        var log = _config.Logger;
